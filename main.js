...
function destroyer (arr) {
  var toExclude = Array.from(arguments).slice(1) // Array
  return arr.filter(function (elt) { // number
  	return toExclude.indexOf(elt) == -1
			// return toExclude.includes(elt);
  })
}
...
function getIndexToIns (arr, num) {
  ...
  let indexOfnum = sortedArray.findIndex(function () {})
}
...
function sumAll (arr) {
  if (arr[0] < arr[1]) {
    var complitedArray = range(arr[0], arr[1])
  } else {
    var complitedArray = range(arr[1], arr[0])
  }

  return complitedArray.reduce(function (a, b) { return a + b })
}
...
function diffArray (arrA, arrB) {
  var newArrayA = arrA.filter(function (elt) { return arrB.indexOf(elt) === -1 })
  var newArrayB = arrB.filter(function (elt) { return arrA.indexOf(elt) === -1 })
  return symmetricDifferenceArray = newArrayA.concat(newArrayB)
}
...
function repeatStringNumTimes (str, num) {
  ...
  /* if (num > 0) {
  	var repeatedString = Array.from({length: num}, function (el, i) { return str })
...
function chunkArrayInGroups (arr, size) {
  var chunkedArray = Array.from({length: Math.floor(length / size)}, function (_, i) { return arr.slice(i * size, i * size + size) })
  return chunkedArray
}
...
function slasher (arr, howMany) {
 /* var arr = Array.from(arr)
  ...
  // OR WITH DECLARATIVE FUNCTIONS
  return arr.filter(function (el) { if (arr.indexOf(el) >= howMany) return el })
}
...
function mutation (arr) {
  ...
  var el1 = arr[0].toLowerCase()
  return secondWord.every(function (c) { return el1.indexOf(c) > -1 })
}
